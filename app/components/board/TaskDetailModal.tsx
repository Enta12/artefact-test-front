...
import { TaskFormData, TaskFormSubmitData } from '../../types/task';
...
const TYPE_STYLE = {
  ...
  IMPROVEMENT: { icon: FiRotateCw, label: 'Amélioration', color: 'text-green-600' },
};
...
const TaskDetailModal = ({ task, onClose }: TaskDetailModalProps) => {
  ...
  const { data: users = [] } = useAuthQuery<User[]>(
    ['project-users', task.projectId],
    `${process.env.NEXT_PUBLIC_API_URL}/projects/${task.projectId}/users`
  );

  const { data: tags = [] } = useAuthQuery<Tag[]>(
    ['project-tags', task.projectId],
    `${process.env.NEXT_PUBLIC_API_URL}/tags/project/${task.projectId}`
  );
  ...
  const createTag = useAuthMutation<Tag, Error, { name: string; color: string; projectId: number }>(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['tags', task.projectId] });
      },
    }
  );

  const updateTask = useAuthMutation(
    (data: TaskFormData) => {
      const submitData: TaskFormSubmitData = {
        title: data.title,
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['columns', task.projectId] });
        setIsEditing(false);
      },
    }
  );

  const deleteTask = useAuthMutation(
    () => ({
      url: `${process.env.NEXT_PUBLIC_API_URL}/tasks/${task.id}`,
      method: 'DELETE',
    }),
          ...
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                {TypeIcon && (
                  <div className={`p-2 rounded-lg bg-gray-100 ${taskType.color}`}>
        ...
        {isEditing ? (
          ...
          <div className="space-y-8">
            <div className="flex flex-wrap items-center gap-3 pb-6 border-b border-gray-200">
              {priority && (
                <div className={`flex items-center gap-2 px-4 py-3 rounded-xl border ${priority.color} shadow-sm`}>
                  {PriorityIcon && <PriorityIcon className="w-5 h-5" />}
                  <span className="font-medium">Priorité {priority.label}</span>
                </div>
              )}

              {isOverdue && (
                <div className="flex items-center gap-2 px-4 py-3 rounded-xl border bg-gradient-to-r from-red-50 to-red-100 text-red-800 border-red-200 shadow-sm animate-pulse">
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {task.assignedTo ? (
                        <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
                          ...
                          <span className="font-medium text-blue-900">{task.assignedTo.name}</span>
                        </div>
                      ) : (
                        ...
                        <div className={`flex items-center gap-3 p-3 rounded-xl border ${
                          ...
                          <div>
                            <div className={`font-medium ${
                              isOverdue ? 'text-red-900' : 'text-green-900'
                            }`}>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-3 text-sm text-gray-600">
                        ...
                        <div>
                          <div>Modifié {formatRelativeTime(task.updatedAt)}</div>
                          <div className="text-xs text-gray-500">
